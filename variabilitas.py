...
data=np.genfromtxt(folder+nama_obj+'.tsv',skip_header=1,dtype=str)
...
for i in range(len(data[0])):
    os.chdir(folder+'RXTE/'+data[0][i][:-5]+'/')
    pca=fits.open(data[0][i]+'_grp.pha')
...
ax1[0].set_title('Variabilitas Obs id RXTE '+nama_obj)
#ax[2].set_ylim(1e-12,2e-8)

count=[]
for i in range(len(soft)):
    if soft[i]>0.5 and soft[i] <1.1 and hard[i]>0.5:
        count.append(data[0][i])
